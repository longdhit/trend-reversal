...
#define OBJPREFIX          "HLine - "




static datetime TimeStamp;
static datetime M15TimeStamp;

static bool haveOrder = false;
static double highD1;
static double lowD1;
...
input string credit = "Trend Reversal";
input string _token = "";
...
int OnInit()
  {
   ...
   highD1 = iHigh(NULL,PERIOD_D1,1);
   lowD1 = iLow(NULL,PERIOD_D1,1);
   ...
   if(iHigh(NULL,PERIOD_D1,0) >= highD1 ||  iLow(NULL,PERIOD_D1,0) <= lowD1)
     {
...
//--- indicator buffers mapping
   gPointCoef = calculatePointCoef();

   HLineCreate(highD1,clrAqua);
   HLineCreate(lowD1,clrRed);

   Comment(StringFormat(" uptrend: %d \n downtrend: %d \n count: %d \n skip: %d", uptrend, downtrend, count, skip));
...
//--- return value of prev_calculated for next call
   if(TimeStamp != iTime(NULL,PERIOD_D1,1))
     {
      TimeStamp = iTime(NULL,PERIOD_D1,1);
      haveOrder = false;
      highD1 = iHigh(NULL,PERIOD_D1,1);
      lowD1 = iLow(NULL,PERIOD_D1,1);
      skip = false;
      ...
      value2 = highD1;
      DrawFibo(OBJPREFIX+Symbol()+"1", time, value1, time, value2);
        DrawFibo(OBJPREFIX+Symbol()+"2", time, value2, time, value1);
      count = 0;
      uptrend= false;
      downtrend =false;
     }
   ...
   if(M15TimeStamp != iTime(NULL,PERIOD_M15,1))
     {
      M15TimeStamp = iTime(NULL,PERIOD_M15,1);
      if(uptrend == true)
        {
         handleUptrend();
        }
   ...
   if(downtrend == false && uptrend == false && Bid >= highD1)
     {
      uptrend = true;
      sendMessage("Chuẩn bị có tín hiệu SELL " +Symbol());
      alert("Chuan bi co tin hieu SELL " + Symbol());
      count = 1;
     }

   if(downtrend == false && uptrend == false && Bid <= lowD1)
     {
      downtrend = true;
      sendMessage("Chuẩn bị có tín hiệu BUY " +Symbol());
      alert("Chuan bi co tin hieu SELL " + Symbol());
      count = 1;
     }
...
void handleUptrend()
  {
   if(count == 2)
     {
      if((iClose(NULL,PERIOD_M15, 1) <= highD1) && (iClose(NULL,PERIOD_M15, 1) < iClose(NULL,PERIOD_M15, 2)))
        {
         sendMessage(StringFormat("%s\r\nSELL: %s\r\nTP: %s\r\nSL: %s",Symbol(),DoubleToStr(Bid,Digits), DoubleToStr(Bid - sqConvertToRealPips(takeprofit),Digits),DoubleToStr(Bid + sqConvertToRealPips(stoploss),Digits)));
         alert(StringFormat("SELL %s: %s - TP: %s - SL: %s",Symbol(),DoubleToStr(Bid,Digits), DoubleToStr(Bid - sqConvertToRealPips(takeprofit),Digits),DoubleToStr(Bid + sqConvertToRealPips(stoploss),Digits)));

         OrderSendReliable(Symbol(),OP_SELL, lot,Bid,3,Bid + sqConvertToRealPips(stoploss),Bid - sqConvertToRealPips(takeprofit),credit,magicNumber,0,clrRed);

        }
      else
        {
         sendMessage("Hủy kèo " +Symbol());
         alert("Huy keo " + Symbol());
        }
      skip = true;
      return;
     }
...
void handleDowntrend()
  {
   if(count == 2)
     {
      if((iClose(NULL,PERIOD_M15, 1) >= lowD1) && (iClose(NULL,PERIOD_M15, 1) > iClose(NULL,PERIOD_M15, 2)))
        {
         sendMessage(StringFormat("%s\r\nBUY: %s\r\nTP: %s\r\nSL: %s",Symbol(),DoubleToStr(Ask, Digits), DoubleToStr(Ask + sqConvertToRealPips(takeprofit),Digits),DoubleToStr(Ask - sqConvertToRealPips(stoploss), Digits)));
         alert(StringFormat("BUY %s - TP: %s - SL: %s",Symbol(),DoubleToStr(Ask, Digits), DoubleToStr(Ask + sqConvertToRealPips(takeprofit),Digits),DoubleToStr(Ask - sqConvertToRealPips(stoploss), Digits)));
         OrderSendReliable(Symbol(),OP_BUY, lot,Ask,3,Ask - sqConvertToRealPips(stoploss),Ask + sqConvertToRealPips(takeprofit),credit,magicNumber,0,clrBlue);
...
//|                                                                  |
//+------------------------------------------------------------------+
bool drawSymbol(double                price=0,              // line price
                const color           clr=clrRed)
  {
   string name = OBJPREFIX +_Symbol +price + "_SYMBOL";
   datetime x1 = iTime(_Symbol, _Period, (WindowFirstVisibleBar() - 2));
   ...
   if(ObjectFind(0, name) != -1)
     {
      ...
      ObjectMove(0, "label_"+name, 0, x1,price);
      return (true);
     }
...
//--- set the priority for receiving the event of a mouse click in the chart
   ...
   ObjectCreate(0,name+"_label",OBJ_TEXT, 0, x1, price);
   ObjectSetText(name+"_label", "n", 14,"Wingdings", clr);
   ObjectSetInteger(chart_ID,name+"_label",OBJPROP_SELECTABLE,selection);
   ObjectSetInteger(chart_ID,name+"_label",OBJPROP_SELECTED,selection);
//--- hide (true) or display (false) graphical object name in the object list
   ObjectSetInteger(chart_ID,name+"_label",OBJPROP_HIDDEN,hidden);
...
bool HLineCreate(
  ...
  {
   string name = OBJPREFIX +_Symbol +price + text;
   datetime x1 = iTime(_Symbol, _Period, (WindowFirstVisibleBar() - 2));
   const long chart_ID=0;        // chart's ID
   const ENUM_LINE_STYLE style=STYLE_SOLID; // line style
   const int             width=2;          // line width
...
//--- set the priority for receiving the event of a mouse click in the chart
   ...
   ObjectCreate(0,name+"_label",OBJ_TEXT, 0, x1, price);
   ObjectSetText(name+"_label", text, 0, NULL,clr);
   ObjectSetInteger(chart_ID,name+"_label",OBJPROP_SELECTABLE,selection);
   ObjectSetInteger(chart_ID,name+"_label",OBJPROP_SELECTED,selection);
//--- hide (true) or display (false) graphical object name in the object list
   ObjectSetInteger(chart_ID,name+"_label",OBJPROP_HIDDEN,hidden);
...
//+------------------------------------------------------------------+
bool tms_send(string message)   // You can set token here for simply usage tms_send("you message");
  {
   ...
   if(IsTesting() || IsOptimization())
      return true;
   if(_tms_last_time_messaged > TimeCurrent())
      return false; // do not send twice at the same candle;

   string spost = StringFormat("message=%s&token=%s&code=MQL",message,_token);
            ...
            if(priceNow != price)
              {
               ...
               if(stoploss != 0)
                  stoploss += M * hasSlippedBy;
               if(takeprofit != 0)
                  takeprofit += M * hasSlippedBy;
   ...
   while(!exitLoop)
     {
      ...
      if(err == ERR_NO_ERROR)
         ...
         if(err == ERR_INVALID_STOPS)
            ...
            if(err == ERR_TRADE_MODIFY_DENIED)
...
//|                                                                  |
//+------------------------------------------------------------------+
double sqConvertToRealPips(double value)
  {
...
//|                                                                  |
//+------------------------------------------------------------------+
double calculatePointCoef()
  {


   int realDigits = (int) MarketInfo(Symbol(), MODE_DIGITS);
   ...
   if(result  ||  err == ERR_NO_RESULT  ||  err == ERR_NO_ERROR)
     {
      if(!OrderSelect(ticket, SELECT_BY_TICKET, MODE_TRADES))
         ...
         if(OrderCloseTime() > 0)    // Then it closed ok
            OrderReliablePrint(fn, "Successful close of Ticket #" + IntegerToString(ticket) + "     [ Last error: " + OrderReliableErrTxt(err) + " ]");
         else
           {
            ...
            if(cmd == OP_BUYSTOP  ||  cmd == OP_SELLLIMIT)
              {
               ...
               if(cmd == OP_BUYLIMIT  ||  cmd == OP_SELLSTOP)
                 {
                  price -= point;
                  if(stoploss > 0)
                     stoploss -= point;
...
int MySymbolConst2Val(string symbol="")
  {
   ...
   if(StringFind(mySymbol, "AUDCAD") != -1)
      return(1);
   if(StringFind(mySymbol, "AUDCHF") != -1)
      return(2);
   if(StringFind(mySymbol, "AUDJPY") != -1)
      return(3);
   if(StringFind(mySymbol, "AUDNZD") != -1)
      return(4);
   if(StringFind(mySymbol, "AUDUSD") != -1)
      return(5);
   if(StringFind(mySymbol, "CADCHF") != -1)
      return(6);
   if(StringFind(mySymbol, "CADJPY") != -1)
      return(7);
   if(StringFind(mySymbol, "CHFJPY") != -1)
      return(8);
   if(StringFind(mySymbol, "EURAUD") != -1)
      return(9);
   if(StringFind(mySymbol, "EURCAD") != -1)
      return(10);
   if(StringFind(mySymbol, "EURCHF") != -1)
      return(11);
   if(StringFind(mySymbol, "EURGBP") != -1)
      return(12);
   if(StringFind(mySymbol, "EURJPY") != -1)
      return(13);
   if(StringFind(mySymbol, "EURNZD") != -1)
      return(14);
   if(StringFind(mySymbol, "EURUSD") != -1)
      return(15);
   if(StringFind(mySymbol, "GBPAUD") != -1)
      return(16);
   if(StringFind(mySymbol, "GBPCAD") != -1)
      return(17);
   if(StringFind(mySymbol, "GBPCHF") != -1)
      return(18);
   if(StringFind(mySymbol, "GBPJPY") != -1)
      return(19);
   if(StringFind(mySymbol, "GBPNZD") != -1)
      return(20);
   if(StringFind(mySymbol, "GBPUSD") != -1)
      return(21);
   if(StringFind(mySymbol, "NZDCAD") != -1)
      return(22);
   if(StringFind(mySymbol, "NZDCHF") != -1)
      return(23);
   if(StringFind(mySymbol, "NZDJPY") != -1)
      return(24);
   if(StringFind(mySymbol, "NZDUSD") != -1)
      return(25);
   if(StringFind(mySymbol, "USDCAD") != -1)
      return(26);
   if(StringFind(mySymbol, "USDCHF") != -1)
      return(27);
   if(StringFind(mySymbol, "USDJPY") != -1)
      return(28);
   if(StringFind(mySymbol, "XAGUSD") != -1)
      return(29);
   if(StringFind(mySymbol, "SILVER") != -1)
      return(29);
   if(StringFind(mySymbol, "XAUUSD") != -1)
      return(30);
   if(StringFind(mySymbol, "GOLD") != -1)
      return(30);

// These symbols were added 26Sep10
   if(StringFind(mySymbol, "AUDDKK") != -1)
      return(31);
   if(StringFind(mySymbol, "AUDNOK") != -1)
      return(32);
   if(StringFind(mySymbol, "AUDSEK") != -1)
      return(33);
   if(StringFind(mySymbol, "CHFNOK") != -1)
      return(34);
   if(StringFind(mySymbol, "EURDKK") != -1)
      return(35);
   if(StringFind(mySymbol, "EURPLN") != -1)
      return(36);
   if(StringFind(mySymbol, "EURSEK") != -1)
      return(37);
   if(StringFind(mySymbol, "EURSGD") != -1)
      return(38);
   if(StringFind(mySymbol, "EURZAR") != -1)
      return(39);
   if(StringFind(mySymbol, "GBPNOK") != -1)
      return(40);
   if(StringFind(mySymbol, "GBPNZD") != -1)
      return(41);
   if(StringFind(mySymbol, "GBPSGD") != -1)
      return(42);
   if(StringFind(mySymbol, "NOKJPY") != -1)
      return(43);
   if(StringFind(mySymbol, "SEKJPY") != -1)
      return(44);
   if(StringFind(mySymbol, "USDAED") != -1)
      return(45);
   if(StringFind(mySymbol, "USDBHD") != -1)
      return(46);
   if(StringFind(mySymbol, "USDDKK") != -1)
      return(47);
   if(StringFind(mySymbol, "USDEGP") != -1)
      return(48);
   if(StringFind(mySymbol, "USDHKD") != -1)
      return(49);
   if(StringFind(mySymbol, "USDJOD") != -1)
      return(50);
   if(StringFind(mySymbol, "USDKWD") != -1)
      return(51);
   if(StringFind(mySymbol, "USDMXN") != -1)
      return(52);
   if(StringFind(mySymbol, "USDNOK") != -1)
      return(53);
   if(StringFind(mySymbol, "USDPLN") != -1)
      return(54);
   if(StringFind(mySymbol, "USDQAR") != -1)
      return(55);
   if(StringFind(mySymbol, "USDSAR") != -1)
      return(56);
   if(StringFind(mySymbol, "USDSEK") != -1)
      return(57);
   if(StringFind(mySymbol, "USDSGD") != -1)
      return(58);
   if(StringFind(mySymbol, "USDTHB") != -1)
      return(59);
   if(StringFind(mySymbol, "USDZAR") != -1)
      return(60);
...
// This varies broker to broker, but that's fine
   ...
   return(SymbolsTotal(false)+1);   // Don't use zero
  }
...
// then lets return the Symbol (EURUSD.lmax instead of just EURUSD).
   if(val == 1)
      return((StringFind(Symbol(), "AUDCAD") != -1) ? Symbol() : "AUDCAD");
   if(val == 2)
      return((StringFind(Symbol(), "AUDCHF") != -1) ? Symbol() : "AUDCHF");
   if(val == 3)
      return((StringFind(Symbol(), "AUDJPY") != -1) ? Symbol() : "AUDJPY");
   if(val == 4)
      return((StringFind(Symbol(), "AUDNZD") != -1) ? Symbol() : "AUDNZD");
   if(val == 5)
      return((StringFind(Symbol(), "AUDUSD") != -1) ? Symbol() : "AUDUSD");
   if(val == 6)
      return((StringFind(Symbol(), "CADCHF") != -1) ? Symbol() : "CADCHF");
   if(val == 7)
      return((StringFind(Symbol(), "CADJPY") != -1) ? Symbol() : "CADJPY");
   if(val == 8)
      return((StringFind(Symbol(), "CHFJPY") != -1) ? Symbol() : "CHFJPY");
   if(val == 9)
      return((StringFind(Symbol(), "EURAUD") != -1) ? Symbol() : "EURAUD");
   if(val == 10)
      return((StringFind(Symbol(), "EURCAD") != -1) ? Symbol() : "EURCAD");
   if(val == 11)
      return((StringFind(Symbol(), "EURCHF") != -1) ? Symbol() : "EURCHF");
   if(val == 12)
      return((StringFind(Symbol(), "EURGBP") != -1) ? Symbol() : "EURGBP");
   if(val == 13)
      return((StringFind(Symbol(), "EURJPY") != -1) ? Symbol() : "EURJPY");
   if(val == 14)
      return((StringFind(Symbol(), "EURNZD") != -1) ? Symbol() : "EURNZD");
   if(val == 15)
      return((StringFind(Symbol(), "EURUSD") != -1) ? Symbol() : "EURUSD");
   if(val == 16)
      return((StringFind(Symbol(), "GBPAUD") != -1) ? Symbol() : "GBPAUD");
   if(val == 17)
      return((StringFind(Symbol(), "GBPCAD") != -1) ? Symbol() : "GBPCAD");
   if(val == 18)
      return((StringFind(Symbol(), "GBPCHF") != -1) ? Symbol() : "GBPCHF");
   if(val == 19)
      return((StringFind(Symbol(), "GBPJPY") != -1) ? Symbol() : "GBPJPY");
   if(val == 20)
      return((StringFind(Symbol(), "GBPNZD") != -1) ? Symbol() : "GBPNZD");
   if(val == 21)
      return((StringFind(Symbol(), "GBPUSD") != -1) ? Symbol() : "GBPUSD");
   if(val == 22)
      return((StringFind(Symbol(), "NZDCAD") != -1) ? Symbol() : "NZDCAD");
   if(val == 23)
      return((StringFind(Symbol(), "NZDCHF") != -1) ? Symbol() : "NZDCHF");
   if(val == 24)
      return((StringFind(Symbol(), "NZDJPY") != -1) ? Symbol() : "NZDJPY");
   if(val == 25)
      return((StringFind(Symbol(), "NZDUSD") != -1) ? Symbol() : "NZDUSD");
   if(val == 26)
      return((StringFind(Symbol(), "USDCAD") != -1) ? Symbol() : "USDCAD");
   if(val == 27)
      return((StringFind(Symbol(), "USDCHF") != -1) ? Symbol() : "USDCHF");
   if(val == 28)
      return((StringFind(Symbol(), "USDJPY") != -1) ? Symbol() : "USDJPY");
   if(val == 29)
      return((StringFind(Symbol(), "XAGUSD") != -1  ||
              StringFind(Symbol(), "SILVER") != -1) ? Symbol() : "XAGUSD");
   if(val == 30)
      return((StringFind(Symbol(), "XAUUSD") != -1  ||
              StringFind(Symbol(), "GOLD")   != -1) ? Symbol() : "XAUUSD");

// These symbols were added 26Sep10
   if(val == 31)
      return((StringFind(Symbol(), "AUDDKK") != -1) ? Symbol() : "AUDDKK");
   if(val == 32)
      return((StringFind(Symbol(), "AUDNOK") != -1) ? Symbol() : "AUDNOK");
   if(val == 33)
      return((StringFind(Symbol(), "AUDSEK") != -1) ? Symbol() : "AUDSEK");
   if(val == 34)
      return((StringFind(Symbol(), "CHFNOK") != -1) ? Symbol() : "CHFNOK");
   if(val == 35)
      return((StringFind(Symbol(), "EURDKK") != -1) ? Symbol() : "EURDKK");
   if(val == 36)
      return((StringFind(Symbol(), "EURPLN") != -1) ? Symbol() : "EURPLN");
   if(val == 37)
      return((StringFind(Symbol(), "EURSEK") != -1) ? Symbol() : "EURSEK");
   if(val == 38)
      return((StringFind(Symbol(), "EURSGD") != -1) ? Symbol() : "EURSGD");
   if(val == 39)
      return((StringFind(Symbol(), "EURZAR") != -1) ? Symbol() : "EURZAR");
   if(val == 40)
      return((StringFind(Symbol(), "GBPNOK") != -1) ? Symbol() : "GBPNOK");
   if(val == 41)
      return((StringFind(Symbol(), "GBPNZD") != -1) ? Symbol() : "GBPNZD");
   if(val == 42)
      return((StringFind(Symbol(), "GBPSGD") != -1) ? Symbol() : "GBPSGD");
   if(val == 43)
      return((StringFind(Symbol(), "NOKJPY") != -1) ? Symbol() : "NOKJPY");
   if(val == 44)
      return((StringFind(Symbol(), "SEKJPY") != -1) ? Symbol() : "SEKJPY");
   if(val == 45)
      return((StringFind(Symbol(), "USDAED") != -1) ? Symbol() : "USDAED");
   if(val == 46)
      return((StringFind(Symbol(), "USDBHD") != -1) ? Symbol() : "USDBHD");
   if(val == 47)
      return((StringFind(Symbol(), "USDDKK") != -1) ? Symbol() : "USDDKK");
   if(val == 48)
      return((StringFind(Symbol(), "USDEGP") != -1) ? Symbol() : "USDEGP");
   if(val == 49)
      return((StringFind(Symbol(), "USDHKD") != -1) ? Symbol() : "USDHKD");
   if(val == 50)
      return((StringFind(Symbol(), "USDJOD") != -1) ? Symbol() : "USDJOD");
   if(val == 51)
      return((StringFind(Symbol(), "USDKWD") != -1) ? Symbol() : "USDKWD");
   if(val == 52)
      return((StringFind(Symbol(), "USDMXN") != -1) ? Symbol() : "USDMXN");
   if(val == 53)
      return((StringFind(Symbol(), "USDNOK") != -1) ? Symbol() : "USDNOK");
   if(val == 54)
      return((StringFind(Symbol(), "USDPLN") != -1) ? Symbol() : "USDPLN");
   if(val == 55)
      return((StringFind(Symbol(), "USDQAR") != -1) ? Symbol() : "USDQAR");
   if(val == 56)
      return((StringFind(Symbol(), "USDSAR") != -1) ? Symbol() : "USDSAR");
   if(val == 57)
      return((StringFind(Symbol(), "USDSEK") != -1) ? Symbol() : "USDSEK");
   if(val == 58)
      return((StringFind(Symbol(), "USDSGD") != -1) ? Symbol() : "USDSGD");
   if(val == 59)
      return((StringFind(Symbol(), "USDTHB") != -1) ? Symbol() : "USDTHB");
   if(val == 60)
      return((StringFind(Symbol(), "USDZAR") != -1) ? Symbol() : "USDZAR");
   return("Unknown");
  }
...
//// Values for levels
  // ObjectSetInteger(chartId,name,OBJPROP_LEVELS,0);
